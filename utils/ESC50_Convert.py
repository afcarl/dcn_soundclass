...
def stereo2mono(data) :
    ...
    if data.ndim != 2 :
        print('You are calling stereo2mono on a non-2D array')
    else : 
...
#----------------
def wav2spect(fname, srate, fftSize, fftHop, dur=None, showplt=False, dcbin=False, framesmulitpleof=1) :
    try:
...
def esc50Wav2Spect(topdir, outdir, dur, srate, fftSize, fftHop, showplt=False, dcbin=False) :
    ...
    for subdir in subdirs :
        ...
        for idx in range(len(fullpaths)) : 
            ...
            D = wav2spect(fullpaths[idx], srate, fftSize, fftHop, dur=None, dcbin=False, showplt=False, framesmulitpleof=K_FRAMEMULTIPLEOF)
...
if (cont=='y'):
	...
	dirs2labelfile(K_SPECTDIR, K_LABELFILE)
else:
	print ('OK, exiting')
